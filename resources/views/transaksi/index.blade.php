            ...
            <main class="flex-1 overflow-y-auto px-8 py-5 space-y-5">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <select name="rekening_id" onchange="this.form.submit()" form="transactionFilters" class="text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2.5 text-slate-600 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="">Semua Rekening</option>
                            @foreach ($accountOptions as $account)
                                <option value="{{ $account }}" @selected(request('rekening_id') === $account)>{{ $account }}</option>
                            @endforeach
                        </select>
                        <select name="kategori" onchange="this.form.submit()" form="transactionFilters" class="text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2.5 text-slate-600 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="">Semua Kategori</option>
                            @foreach ($categoryOptions as $category)
                                <option value="{{ $category }}" @selected(request('kategori') === $category)>{{ ucfirst($category) }}</option>
                            @endforeach
                        </select>
                        <select name="status" onchange="this.form.submit()" form="transactionFilters" class="text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2.5 text-slate-600 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="flex justife-between gap-3">
                        <button
                                ...
                                class="flex items-center gap-2 bg-white dark:bg-slate-900 hover:bg-red-50 dark:hover:bg-red-950/50 text-red-500 border border-red-200 dark:border-red-900/50 hover:border-red-300 text-sm font-medium px-4 py-2.5 rounded-lg transition-colors"
                            >
                        ...
                        <button onclick="openModal()" type="button" class="inline-flex items-center gap-2 bg-slate-950 dark:bg-blue-600 hover:bg-slate-800 dark:hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
